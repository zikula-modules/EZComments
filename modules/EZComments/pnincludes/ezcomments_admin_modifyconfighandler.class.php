...
class EZComments_admin_modifyconfighandler
{
    function initialize(&$pnRender)
    {
        ...
        $pnRender->assign('templates', $templates);
    ...
    function handleCommand(&$pnRender, &$args)
    {
        ...
        if ($args['commandName'] == 'submit') {
            ...
            if(empty($data['ezcomments_itemsperpage'])) {
                $ifield = & $pnRender->pnFormGetPluginById('ezcomments_itemsperpage');
                $ifield->setError(DataUtil::formatForDisplay(_EZCOMMENTS_MISSINGVALUE));
                $ok = false;
            }
            if(empty($data['ezcomments_modlinkcount'])) {
                $ifield = & $pnRender->pnFormGetPluginById('ezcomments_modlinkcount');
                $ifield->setError(DataUtil::formatForDisplay(_EZCOMMENTS_MISSINGVALUE));
                $ok = false;
            }
            if(empty($data['ezcomments_blacklinkcount'])) {
                $ifield = & $pnRender->pnFormGetPluginById('ezcomments_blacklinkcount');
                $ifield->setError(DataUtil::formatForDisplay(_EZCOMMENTS_MISSINGVALUE));
                $ok = false;
            }
            if(empty($data['ezcomments_feedcount'])) {
                $ifield = & $pnRender->pnFormGetPluginById('ezcomments_feedcount');
                $ifield->setError(DataUtil::formatForDisplay(_EZCOMMENTS_MISSINGVALUE));
                $ok = false;
            }
            if(empty($data['ezcomments_commentsperpage'])) {
                $ifield = & $pnRender->pnFormGetPluginById('ezcomments_commentsperpage');
                $ifield->setError(DataUtil::formatForDisplay(_EZCOMMENTS_MISSINGVALUE));
                $ok = false;
            }
